...
import BidModifiedListButtons from "./BidModifiedButtons";
...
import { getFUFinBidModCancelAction } from "../../../../api/bids";
...
export default function BidsModifiedList(props) {
  ...
  const handleCloseOtp = async () => {
    ...
    let data = JSON.stringify({
      finBidPlaceAction: bidAction,
      modifyCancelBids: RequestArray,
    });
    const ModifyBidRes = await getFUFinBidModCancelAction(data);
    setplaceBidRespo(ModifyBidRes.data);
    if (ModifyBidRes.data.astrid) {
      handleOpen();
      setRows(
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        if (!commodityFilterValue) {
          //resolve(PlaceBids);
          resolve(props.bidModifyRes ? props.bidModifyRes : []);
          return;
        }
        resolve();
        //     PlaceBids.filter(
  ...
  const resetFilter = () => {
    setRows(props.bidModifyRes ? props.bidModifyRes : []);
  ...
  const refreshData = () => {
    props.refreshApiData({ refresh: "Bid Modify" });
  };
      ...
      renderCell: (params) => (
        <>
          ...
          <img
            ...
            onClick={() => handleModifyBidsingle(params.id)}
          />
          <img
            style={{ cursor: "pointer" }}
            ...
            onClick={() => handleCancelBidsingle(params.id)}
          />
        </>
      ),
    },
  ...
  const handleCellEditCommit = React.useCallback(async (params) => {
    try {
      ...
      // console.log("@final@",Number.isInteger(params.value),"@@",params.value)
  ...
  const handleCancelBidsingle = async (rowid) => {
    let newRowsset = rows;
  ...
  const onAutoChange = async (Newdata) => {
    ...
    let modifyFu = await fetchModifyFuNumber(
      Newdata,
      fromDate,
      toDate,
      fuArray
    ...
    let NewRows = await ApiBidModifyFuResponse(modifyFu, fromDate, toDate);
    if (NewRows) {
    ...
    if (_e === "t") {
      if (fromDate) {
        if (fromDate <= _date) {
          ...
          let NewRows = await ApiBidModifyFuResponse(modifyFu, fromDate, _date);
          if (NewRows) {
            let fetchBidCountResponse = await fetchBidCount(
              NewRows.furow,
              NewRows.fuNumArray
  ...
  const handleModifyBidsingle = async (rowid) => {
    let newRowsset = rows;
    let selRow = newRowsset.filter((row) => row.id === rowid);
    setSelectedRows(selRow);
    if (selectedRows && selectedRows.length > 0) {
          ...
          <Grid container spacing={2}>
            ...
            <Grid item>
              ...
              <Button
                ...
                startIcon={
                  <img
                    src={_dark ? CancelBidBtnlight : CancelBidBtn}
                    className={classes.imgCls}
                    alt="Bid"
                  />
                }
