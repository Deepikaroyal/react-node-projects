...
export function generateOptions(
  ...
  const options = {
    method,
    url: `${APIBaseUrl}${port}/${url}`,
  };
  //console.log('url', url)
  //if (url !== 'authapi/auth/apilogin' && url !== 'authapi/auth/apikeygen') {
  if (url !== "authapi/auth/signin" || url !== "authapi/auth/platformsignin") {
    options.headers = {
      "Content-Type": formType,
      Authorization: `Bearer ${
        store?.getState()?.auth?.token || sessionStorage.token
      }`,
      logintype: "JWT",
      //'logintype': 'API',
    };
